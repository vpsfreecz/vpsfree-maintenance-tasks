...
PLAYGROUND_ENVIRONMENT_LABEL = 'Playground'
...
module TransactionChains
  module Maintenance
    ...
    class Custom < TransactionChain
      ...
      def link_chain
        playground_env = ::Environment.find_by!(label: PLAYGROUND_ENVIRONMENT_LABEL)
        sent = 0
        ignored_no_active_non_playground_vpses = 0
        ignored_no_vpses_with_cgroups_v1 = 0

        eligible_users.each do |user|
          active_vpses = user_active_vpses(user, playground_env.id)
          if active_vpses.empty?
            puts "Ignoring user #{user.id} #{user.login}: no active non-Playground VPSes"
            ignored_no_active_non_playground_vpses += 1
            next
          end
          ...
          if vpses_on_v1.empty?
            puts "Ignoring user #{user.id} #{user.login}: no non-Playground VPSes with cgroups v1"
        ...
        puts "Prepared #{sent} emails (ignored_no_active_non_playground_vpses=#{ignored_no_active_non_playground_vpses}, ignored_no_vpses_with_cgroups_v1=#{ignored_no_vpses_with_cgroups_v1})"
      ...
      def user_active_vpses(user, playground_env_id)
        user.vpses.includes(:os_template, node: %i(node_current_status location)).joins(
          node: %i(node_current_status location),
        ).where(
          vpses: { object_state: ::Vps.object_states[:active] },
        ).where.not(
          locations: { environment_id: playground_env_id },
        ).order('vpses.id').to_a
      end
